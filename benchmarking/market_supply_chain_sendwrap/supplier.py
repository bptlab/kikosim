...
# ═══════════════════════════════════════════════════════════════════
# SEND WRAPPERS - For outbound messages
# ═══════════════════════════════════════════════════════════════════

async def send_accept(message: Accept):
    """Wrapper around sending Accept messages."""
    await adapter.send(message)

async def send_reject(message: Reject):
    """Wrapper around sending Reject messages."""
    await adapter.send(message)

async def send_ship_package(message: ShipPackage):
    """Wrapper around sending ShipPackage messages."""
    await adapter.send(message)
...
async def receive_own_accept(msg):
    ...
    if order_response == "ACCEPTED":
        ...
        # Use wrapper for sending
        await send_ship_package(package_msg)
        # Log with business message emoji and all properties
