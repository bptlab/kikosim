...
def mk_deferred(
    ...
    def outer(func):
        async def core(msg, *a, **kw):            
            ...
            async def execute_business_logic():
                ...
                try:
                    ...
                    log.error(f"💥 {prefix}: business logic failed for {task_id}: {e}")
                    raise
        ...
        adapter_decorator(core)   # ← register with BSPL (returns None)
        return core               # ← keep a *callable* reference for your code
    return outer
...
def create_resource_manager(
    ...
    *,
    ...
    pending_tasks : dict[str, Awaitable]
        Local storage that maps *taskID* → coroutine so the CompleteTask handler
    ...
    def create_task_sender_factory(prefix: str):
        ...
        if len(task_setting) == 2:
            ...
            if std_days == 0:
                ...
                duration = f"{mean_days}d±{std_days}d"
        else:
...
def parse_duration(duration_str: str) -> tuple[float, float]:
    ...
    - "1.5d±0.5d" - Normal distribution with mean=1.5d, std_dev=0.5d
    - "2h±30m" - Normal distribution with mean=2h, std_dev=30m
...
def sample_duration(duration_str: str) -> float:
    ...
    Args:
        duration_str: Duration string (e.g., "1.5d±0.5d" or "1.5d")
...
DURATION_PATTERNS = {
    "fast_service": "0.5d±0.1d",               # Fast service: 0.5 days ± 2.4 hours
    "standard_service": "1d±0.3d",             # Standard service: 1 day ± 7.2 hours  
    "slow_service": "2d±0.5d",                 # Slow service: 2 days ± 12 hours
    "manufacturing": "1.5d±0.4d",              # Manufacturing: 1.5 days ± 9.6 hours
    "shipping": "3d±0.8d",                     # Shipping: 3 days ± 19.2 hours
    "approval": "0.5d±0.15d",                  # Approval: 0.5 days ± 3.6 hours
